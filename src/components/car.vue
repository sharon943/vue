      ...
      <section v-if="this.status" class="sec">
        ...
        <iscroll-view class="center" :options="{preventDefault: false}">
          ...
          <div class="bottom">
            ...
            <div class="foot">
              <img src="../assets/img/button_03.png" alt="" @click="sendcar">
            </div>
          </div>
      </section>
...
<script>
  export default {
    ...
    methods:{
        ...
        sendcar(){
            this.$http.post('/home/addorder.php',{car:this.car}).then(mess=>{
                if(mess.body=="yes"){
                    this.$router.replace({name:'success'});
                    localStorage.removeItem('car')
                }else{
                    this.$router.push({name:'unsuccess'});
                }
            });
      },
      down(index){
            this.car[index].count--;
            if(this.car[index].count==0){
